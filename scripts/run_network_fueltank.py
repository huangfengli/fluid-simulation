...
def main():
    ...
    parser.add_argument("--scene-id",
                        type=str,
                        default=None,
                        help="Validation scene id to export, e.g. sim_0201. Defaults to the first scene in --zst_dir.")
    ...
    if frames == []:
        print("No frame for scene_id:", selected_scene_id)
        print("Available scene ids:", ", ".join(available_scene_ids))
        return

    num_steps = min(args.num_steps, len(frames))
    print("Exporting scene_id:", selected_scene_id, "frames:", len(frames),
          "num_steps:", num_steps)

    return run_sim_torch(args.weights, scene,
                             num_steps, args.output, args, frames)
