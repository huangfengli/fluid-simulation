...
def run_sim_torch(trainscript_module, weights_path, scene, num_steps,
    ...
    state_dict = weights.get("model") if isinstance(weights, dict) else weights
    model.load_state_dict(state_dict) #如果是自己训练的模型就需要.get("model")
    model.to(device)
...
def main():
    ...
    trainscript_dir = os.path.dirname(os.path.abspath(args.trainscript))
    module_name = os.path.splitext(os.path.basename(args.trainscript))[0]
    sys.path.insert(0, trainscript_dir)
    sys.path.append('.')
