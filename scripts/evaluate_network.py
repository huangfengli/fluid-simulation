...
def evaluate_tf(model, val_dataset, frame_skip, fluid_errors=None, scale=1):
    ...
    store_fluid_errors = fluid_errors is not None
...
def evaluate_torch(model,
    ...
    for data in val_dataset:
        ...
        if data['frame_id0'][0] % skip == 3:
            ...
            if len(
                ...
                frame1_id = frames[1]['frame_id0'][0]
                frame2_id = frames[2]['frame_id0'][0]
                box = to_device_tensor(frames[0]['box'][0])
                box_normals = to_device_tensor(frames[0]['box_normals'][0])
                gt_pos1 = frames[1]['pos0'][0]
                gt_pos2 = frames[2]['pos0'][0]

                inputs = (to_device_tensor(frames[0]['pos0'][0]),
                          to_device_tensor(frames[0]['vel0'][0]), None,
                          box, box_normals)
                out1 = model(inputs)
                if isinstance(out1, (tuple, list)) and len(out1) == 3:
                    pr_pos1, pr_vel1, _ = out1
                else:
                    pr_pos1, pr_vel1 = out1

                inputs = (pr_pos1, pr_vel1, None, box, box_normals)
                out2 = model(inputs)
                if isinstance(out2, (tuple, list)) and len(out2) == 3:
                    pr_pos2, pr_vel2, _ = out2
                else:
                    pr_pos2, pr_vel2 = out2

                if use_gpu_metrics:
                    gt_pos1_tensor = to_device_tensor(gt_pos1)
                    gt_pos2_tensor = to_device_tensor(gt_pos2)
                    err_n1_values.append(
                        torch.linalg.norm(scale * pr_pos1 -
                                          scale * gt_pos1_tensor,
                                          dim=-1).mean().item())
                    err_n2_values.append(
                        torch.linalg.norm(scale * pr_pos2 -
                                          scale * gt_pos2_tensor,
                                          dim=-1).mean().item())
                    if store_fluid_errors:
                        fluid_errors.add_errors(
                            scene_id, frame0_id, frame1_id,
                            scale * pr_pos1.cpu().detach().numpy(),
                            scale * gt_pos1)
                        fluid_errors.add_errors(
                            scene_id, frame0_id, frame2_id,
                ...
                if compute_emd:
                    emd1 = wasserstein_distance(
                        pr_pos1.cpu().detach().numpy().flatten(),
                        gt_pos1.flatten())
                    emd2 = wasserstein_distance(
                        pr_pos2.cpu().detach().numpy().flatten(),
                        gt_pos2.flatten())
    ...
    result = {}
    if err_n1_values:
        result['err_n1'] = float(np.mean(err_n1_values))
        result['err_n2'] = float(np.mean(err_n2_values))
    else:
        result['err_n1'] = np.mean([
            v['mean'] for k, v in fluid_errors.errors.items()
            if k[1] + 1 == k[2]
        ]) #第一帧误差
        result['err_n2'] = np.mean([
            v['mean'] for k, v in fluid_errors.errors.items()
    ...
    if compute_emd:
        result['emd_n1'] = float(np.mean(emd_distance1))
        result['emd_n2'] = float(np.mean(emd_distance2))
    print(result)
    print('done')
    ...
    with torch.no_grad():
        for data in val_dataset:
            ...
            if last_scene_id is None or last_scene_id != scene_id:
                print(scene_id, end=' ', flush=True)
                last_scene_id = scene_id
                box = to_device_tensor(data['box'][0])
                box_normals = to_device_tensor(data['box_normals'][0])
                init_pos = to_device_tensor(data['pos0'][0])
                init_vel = to_device_tensor(data['vel0'][0])
...
def eval_checkpoint(checkpoint_path, val_files, fluid_errors, options, cfg):
    ...
    if checkpoint_path.endswith('.index'):
        ...
        if 'model' in checkpoint:
            incompatible = model.load_state_dict(checkpoint['model'],
                                                 strict=False) #读取训练好的模型
        else:
